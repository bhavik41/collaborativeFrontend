...
const MessageArea: React.FC<MessageAreaProps> = ({ messages, setMessages }) => {
    ...
    const handleSendMessage = () => {
        if (message.trim() && user) {
            const newMessage: Message = { sender: user.email, message, type: "outgoing" };
    ...
    const renderMessageContent = (msg: Message) => {
        if (msg.sender === "AI") {
            ...
            // Handle Markdown and Code Syntax Highlighting
            return (
            ...
            <div className="conversation-area h-[calc(100vh-56px)] flex flex-col">
                ...
                >
                    {messages.map((msg, index) => (
                        <div
                            key={index}
                            className={`message flex flex-col p-2 rounded-md ${msg.sender === 'AI' ? 'max-w-96' : 'max-w-52'} ${msg.type === "incoming"
                                ? "bg-slate-50 w-fit"
